...
- **PowerPoint Presentation (in docs folder):** https://github.com/iamsreecharan/UMBC-DATA606-Capstone/blob/main/docs/Data606_Final_Presentation%20(1).pptx
- **YouTube Demo Video:** https://www.youtube.com/watch?v=bEl9Zu8hyFg
...
- Indicates conservative delivery estimates and an opportunity to improve time predictions.
...
**Visualization:** Plotly bar chart showing average delay rate by customer state.
...
**Visualization:** Line chart showing average delay rate by month.
...
- Product dimensions (weight, length, height, width) are strongly correlated.  
- Freight value vs. delay shows weak correlation.  
- Delivery delay vs. delay days shows an expected strong relationship.  
...
- Examples include freight > 300 R$ or delay days > 100.  
- Outliers were retained for now and evaluated during modeling.
...
2. **Regional Differences Matter**  
   Geography strongly influences delivery performance, so spatial features are essential.  

3. **Seasonality is Visible**  
   Delays peak in March and December, so temporal variables can improve forecasts.  
...
3. **Outlier Treatment**  
   - Cap or scale extreme freight and delay values.  
4. **Model Preparation**  
...
# 5. Machine Learning (ML) Implementation and Results  

## 5.1 Modeling Goal  
The ML stage was designed to answer two practical questions:  
1. Will this order be late or on time?  
2. If it is late, how many days late might it be?  

To support both decisions, I used a two-model setup: a classification model for delay risk and a regression model for delay days.

---

## 5.2 Feature Engineering for ML  
Before training, the final dataset was prepared with features that reflect real delivery behavior:

- **Geospatial distance** between seller and customer using latitude and longitude (Haversine distance).  
- **Temporal features** such as purchase month and weekday, since delay risk changes with seasonal demand.  
- **Estimated lead time** (how many days between purchase and estimated delivery date).  
- **Product features** like weight and dimensions, since bulky items can increase handling time.  
- **Freight value** as a shipping-cost proxy.  
- **Location features** like seller_state and customer_state to capture regional logistics differences.

Categorical fields were encoded for modeling, and numeric variables were cleaned and scaled where needed.

---

## 5.3 Train/Test Strategy and Imbalance Handling  
Because delayed orders are the minority class (around 10 percent), the dataset is imbalanced.  
To avoid a model that predicts “on time” for almost everything, I used **SMOTE** to oversample the delayed class during training.  

The data was split into training and test sets so evaluation was performed on unseen orders.

---

## 5.4 Classification Model (Predict Late vs On-Time)  

### 5.4.1 Model Choice  
**Model used:** XGBoost Classifier  
I selected XGBoost because it handles non-linear relationships well and usually performs strongly on structured tabular data.

### 5.4.2 Evaluation Metrics  
**Results on the test set:**  
- **Accuracy:** 0.78  
- **ROC-AUC:** 0.76  
- **Recall for delayed class:** 0.59  

These results show that the model can identify a meaningful share of delayed orders, which is important for early warning.
...
|---|---:|---:|
| Actual On-time (0) | 16,670 | 4,302 |
| Actual Late (1) | 599 | 845 |

Interpretation:  
- The model catches many late orders (845 true positives).  
- Some late orders are still missed (599 false negatives), which is the main area to improve because missed delays reduce the value of early alerts.

---

## 5.5 Regression Model (Predict Delay Days)  

### 5.5.1 Why Regression is Needed  
Even when an order is predicted as delayed, stakeholders still want a practical estimate:  
“How late will it be?”  
So the second model estimates the number of days late.

### 5.5.2 Model Choice  
**Model used:** XGBoost Quantile Regression (median prediction)  
Delay days are not normally distributed and have outliers, so predicting the median gives more stable results.

### 5.5.3 Regression Performance  
- **MAE (Mean Absolute Error):** approximately **5.1 days**

Interpretation:  
On average, the predicted delay is within about 5 days of the true delay. This is useful for customer communication and operational planning, especially for high-risk routes and peak months.

---

## 5.6 Feature Importance (High-Level Insight)  
From the modeling behavior and EDA patterns, the most influential predictors were:  
- Seller–customer distance  
- Estimated delivery lead time  
- Customer and seller state (regional effects)  
- Seasonal timing (month)  
- Product weight and size  
- Freight value (minor compared to distance and region)

---

## 5.7 Streamlit Demo Application  
To make the project usable, a Streamlit demo application was created that:  
- predicts whether the order will be delayed  
- estimates the number of days late  
- shows a seller-to-customer route visualization  
- outputs an expected arrival date estimate  

This demo supports the “business use” side of the project, not just model accuracy.

---

# 6. Conclusion  

This capstone project built an end-to-end workflow to predict delivery delays in Brazil’s e-commerce supply chain using the Olist dataset.  
The final solution includes:  
- a classification model (XGBoost) to flag late orders early  
- a regression model (quantile regression) to estimate how many days late  
- a Streamlit demo to communicate results in a simple and practical way  

The results show that delivery delays are not random. They follow patterns connected to distance, region, seasonal demand, and product characteristics. With prediction-driven planning, platforms and logistics teams can improve customer trust and reduce delay impact.

---

# 7. Limitations  

- The dataset covers **2016–2018**, so patterns may differ in newer years.  
- Geolocation is approximated using ZIP-code centroids, not exact addresses.  
- External variables like weather, traffic, strikes, and carrier disruptions are not included.  
- The delayed class is a minority (around 10 percent), so prediction is harder and requires careful evaluation beyond accuracy.  
- Some outliers and extreme cases remain difficult to predict, especially when delays are caused by outside events not present in the data.

---

# 8. Future Research Direction  

- Add external signals like weather and traffic to improve predictions.  
- Validate the model on newer data, including post-pandemic delivery behavior.  
- Explore route clustering and network-based features to capture logistics corridors.  
- Improve recall for delayed orders by tuning threshold and cost-sensitive learning.  
- Build a real-time scoring pipeline so orders can be flagged as soon as they are placed.  
